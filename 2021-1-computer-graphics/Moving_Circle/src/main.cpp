...
uint				NUM_TESS = 30;		// initial tessellation factor of the circle as a polygon
extern uint			CIR_NUM = 20;
static const uint   MIN_NUM = 20;
static const uint   MAX_NUM = 100;
...
#endif
auto	circles = std::move(create_circles(CIR_NUM, t, window_size));
...
void update()
{
	...
	void update_cir_num(); // forward declaration
	if(b) update_cir_num(); 
}
...
void render()
{
	...
	{
		// per-circle update
		c.update(t, CIR_NUM, window_size, circles, i, passed_t);
...
void reshape( GLFWwindow* window, int width, int height )
{
	...
	if (width / 16 * 9 < height) {
		...
		glViewport(0, (height-new_height)/2, width, (height+new_height)/2);
		window_size = ivec2(width, new_height);
	}
	else {
		int new_width = height * 16 / 9;
		glViewport((width- new_width)/2, 0, (width+new_width)/2, height);
		window_size = ivec2(new_width, height);
	}
	*/
...
void update_cir_num()
{
	uint n = CIR_NUM; if(b.add) n++; if(b.sub) n--;
	if(n== CIR_NUM ||n<MIN_NUM||n>MAX_NUM) return;
	CIR_NUM = n;
	circles = std::move(create_circles(CIR_NUM, t, window_size));
	passed_t = t;
	printf( "> CIR_NUM = % -4d\r", CIR_NUM );
}
