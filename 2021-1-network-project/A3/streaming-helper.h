#ifndef UDP_REL_ECHO_HELPER_H
#define UDP_REL_ECHO_HELPER_H
 ...
 */
class UdpRelEchoServerHelper
{
public:
  /**
   * Create UdpRelEchoServerHelper which will make life easier for people trying
   * to set up simulations with echos.
   *
   * \param port The port the server will wait on for incoming packets
   */
  UdpRelEchoServerHelper (uint16_t port);
 ...
 */
class UdpRelEchoClientHelper
{
public:
  /**
   * Create UdpRelEchoClientHelper which will make life easier for people trying
   ...
   */
  UdpRelEchoClientHelper (Address ip, uint16_t port);
  /**
   * Create UdpRelEchoClientHelper which will make life easier for people trying
   * to set up simulations with echos. Use this variant with addresses that do
   ...
   */
  UdpRelEchoClientHelper (Address addr);
  ...
  /**
   * Given a pointer to a UdpRelEchoClient application, set the data fill of the 
   ...
   *
   * \param app Smart pointer to the application (real type must be UdpRelEchoClient).
   ...
   *
   * \param node The Ptr<Node> on which to create the UdpRelEchoClientApplication.
   *
   ...
   *
   * \param nodeName The name of the node on which to create the UdpRelEchoClientApplication
   *
   * \returns An ApplicationContainer that holds a Ptr<Application> to the 
   *          application created
   */
...
private:
  /**
   * Install an ns3::UdpRelEchoClient on the node configured with all the
   * attributes set with SetAttribute.
   *
   * \param node The node on which an UdpRelEchoClient will be installed.
   * \returns Ptr to the application installed.
   */
  Ptr<Application> InstallPriv (Ptr<Node> node) const;
  ObjectFactory m_factory; //!< Object factory.
