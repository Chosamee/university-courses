...
class KNN:
    ...
    def predict(self, neighbors, labels, test_data):
        ...
        for i in range(test_num):
            similarity_set.clear()
            for j in range(neighbors_num):
                similarity_set.append(self.nearness(test_data[i], neighbors[j]))
            sim_label = list(zip(similarity_set, labels))
            if self.metric == 'c':
                sim_label.sort(key=lambda x: x[0], reverse=True)
            else:
                sim_label.sort(key=lambda x: x[0], reverse=False)
            check_list = sim_label[:self.k]
            print(check_list)

            labels_dic.clear()
            for k in range(len(check_list)):
                if check_list[k][1] in labels_dic:
                    labels_dic[check_list[k][1]] += 1
                else:
                    labels_dic[check_list[k][1]] = 1
            print(labels_dic)
            aa.clear()
            for l in labels_dic.keys():
                if labels_dic[l] == max(labels_dic.values()):
                    aa.append(l)

            pred_label = random.choice(aa)
            print(pred_label)
    ...
    def nearness(self, x, y):
        ...
        if self.metric == 'c':
            ...
            for i in range(len(x)):
                ...
                bunmo_x += x[i] ** 2
                bunmo_y += y[i] ** 2
            near = bunja / (math.sqrt(bunmo_x) * math.sqrt(bunmo_y))
